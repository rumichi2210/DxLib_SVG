...
// 便利関数
extern	int				GetResourceInfo(			const TCHAR *ResourceName,                            const TCHAR *ResourceType,                            void **DataPointerP , size_t *DataSizeP ) ;		// 指定の名前、タイプのリソースのアドレスとサイズを取得する( 戻り値  -1:失敗  0:成功 )
extern	int				GetResourceInfoWithStrLen(	const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength, void **DataPointerP , size_t *DataSizeP ) ;		// 指定の名前、タイプのリソースのアドレスとサイズを取得する( 戻り値  -1:失敗  0:成功 )
...
extern	int			GetDefaultState(				int *SizeX, int *SizeY, int *ColorBitDepth, int *RefreshRate DEFAULTPARAM( = NULL ) , int *LeftTopX DEFAULTPARAM( = NULL ) , int *LeftTopY DEFAULTPARAM( = NULL ) , int *PixelSizeX DEFAULTPARAM( = NULL ) , int *PixelSizeY DEFAULTPARAM( = NULL ) , int *XDpi DEFAULTPARAM( = NULL ) , int *YDpi DEFAULTPARAM( = NULL ) ) ;		// 起動時のデスクトップの画面情報を取得する( SizeX:デスクトップの横解像度を格納する変数のポインタ  SizeY:縦解像度を格納する変数のポインタ  ColorBitDepth:画面カラービット数を格納する変数のポインタ  RefreshRate:デスクトップのリフレッシュレートを格納する変数のポインタ  LeftTopX:デスクトップの左上Ｘ座標を格納する変数のポインタ  LeftTopY:デスクトップの左上Ｙ座標を格納する変数のポインタ  PixelSizeX:OSの拡大率設定の影響を受けないデスクトップの横解像度を格納する変数のポインタ  PixelSizeY:OSの拡大率設定の影響を受けないデスクトップの縦解像度を格納する変数のポインタ  XDpi:X軸のDPI  YDpi:Y軸のDPI )
extern	int			GetMonitorDpi(					int *XDpi, int *YDpi, int MonitorIndex DEFAULTPARAM( = -1 ) ) ;			// モニターのDPIを取得する
...
extern	int			GetWindowSizeChangeEnableFlag(	int *FitScreen DEFAULTPARAM( = NULL ) ) ;				// ウインドウのサイズを変更できる設定になっているかどうかを取得する( 戻り値  TRUE:変更できるようになっている  FALSE:変更できないようになっている )
extern	double		GetWindowSizeExtendRate(		double *ExRateX DEFAULTPARAM( = NULL ) , double *ExRateY DEFAULTPARAM( = NULL ) ) ;		// 描画画面のサイズに対するウインドウサイズの比率を取得する( 戻り値は ExRateX に代入される値と同じです )
...
extern	int			GetWindowUserCloseFlag(			int StateResetFlag DEFAULTPARAM( = FALSE ) ) ;			// メインウインドウの閉じるボタンが押されたかどうかを取得する
extern	int			CheckWindowMaximizeButtonInput(	int StateResetFlag DEFAULTPARAM( = TRUE ) ) ;			// メインウインドウの最大化ボタンが押されたかどうかを取得する、SetWindowMaximizeButtonBehavior( 1 ); が実行されている場合のみ有効な関数( StateResetFlag  TRUE = 押された状態をリセットする   FALSE = 押された状態をリセットしない )
...
extern	int			SetWindowTextDX(						const TCHAR *WindowText                          ) ;								// メインウインドウのウインドウテキストを変更する
...
extern	int			SetWindowZOrder(						int ZType /* = DX_WIN_ZTYPE_TOP 等 */ , int WindowActivateFlag DEFAULTPARAM( = TRUE ) ) ;	// メインウインドウの奥行き位置を変更する
extern	int			SetWindowSizeChangeEnableFlag(			int Flag, int FitScreen DEFAULTPARAM( = TRUE ) ) ;									// メインウインドウの端を掴んでウインドウのサイズを変更できるようにするかどうかの設定を行う( Flag:変更を可能にするかどうか( TRUE:可能にする  FALSE:不可能にする( デフォルト ) )  FitScreen:ウインドウのクライアント領域に画面をフィットさせる(拡大させる)かどうか  TRUE:フィットさせる  FALSE:フィットさせない )
extern	int			SetWindowSizeExtendRate(				double ExRateX, double ExRateY DEFAULTPARAM( = -1.0 ) ) ;							// 描画画面のサイズに対するメインウインドウサイズの比率を設定する( ExRateY がマイナスの場合は ExRateX の値が ExRateY にも使用されます )
...
extern	int			SetSysCommandOffFlag(					int Flag , const TCHAR *HookDllPath DEFAULTPARAM( = NULL ) ) ;						// タスクスイッチを有効にするかどうかを設定する
extern	int			SetSysCommandOffFlagWithStrLen(			int Flag , const TCHAR *HookDllPath DEFAULTPARAM( = NULL ) , size_t HookDllPathLength DEFAULTPARAM( = 0 ) ) ;	// タスクスイッチを有効にするかどうかを設定する
...
extern	int			SetUseBorderlessWindowFlag(				int Flag ) ;																		// メインウインドウをボーダーレスウィンドウにするかどうかを設定する( TRUE:ボーダーレスウィンドウ　FALSE:通常ウィンドウ )
...
// ツールバー関係
extern	int			SetupToolBar(				const TCHAR *BitmapName,                          int DivNum, int ResourceID DEFAULTPARAM( = -1 ) ) ;					// ツールバーの準備、BitmapName に NULL, ResourceID に -1 を渡すとツールバーを解除( BitmapName:ツールバーのボタンに使用する画像ファイルパス、ResourceID に -1 以外を渡す場合は NULL にする  DivNum:ボタン画像中のボタンの数  ResourceID:ツールバーのボタンに使用するビットマップリソースのＩＤ、BitmapName に NULL を渡すとこの引数が使用される )
extern	int			SetupToolBarWithStrLen(		const TCHAR *BitmapName, size_t BitmapNameLength, int DivNum, int ResourceID DEFAULTPARAM( = -1 ) ) ;					// ツールバーの準備、BitmapName に NULL, ResourceID に -1 を渡すとツールバーを解除( BitmapName:ツールバーのボタンに使用する画像ファイルパス、ResourceID に -1 以外を渡す場合は NULL にする  DivNum:ボタン画像中のボタンの数  ResourceID:ツールバーのボタンに使用するビットマップリソースのＩＤ、BitmapName に NULL を渡すとこの引数が使用される )
...
extern	int			AddMenuItem_ID(						int ParentItemID, const TCHAR *NewItemName,                           int NewItemID DEFAULTPARAM( = -1 ) ) ;		// メニューに選択項目を追加する
extern	int			AddMenuItem_IDWithStrLen(			int ParentItemID, const TCHAR *NewItemName, size_t NewItemNameLength, int NewItemID DEFAULTPARAM( = -1 ) ) ;		// メニューに選択項目を追加する
...
#endif // DX_COMPILE_TYPE_C_LANGUAGE
extern	int			GetMenuItemIDDX(					const TCHAR *ItemName                        ) ;																	// メニューの項目名から項目識別番号を取得する
...
#endif // DX_COMPILE_TYPE_C_LANGUAGE
extern	int			LoadGraphToResource2(				const TCHAR *ResourceName,                            const TCHAR *ResourceType                            ) ;																				// 画像リソースからグラフィックハンドルを作成する
extern	int			LoadGraphToResourceWithStrLen2(		const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength ) ;																				// 画像リソースからグラフィックハンドルを作成する
extern	int			LoadDivGraphToResource2(			const TCHAR *ResourceName,                            const TCHAR *ResourceType,                            int AllNum, int XNum, int YNum, int   XSize, int   YSize, int *HandleArray ) ;	// 画像リソースを分割してグラフィックハンドルを作成する
extern	int			LoadDivGraphToResourceWithStrLen2(	const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength, int AllNum, int XNum, int YNum, int   XSize, int   YSize, int *HandleArray ) ;	// 画像リソースを分割してグラフィックハンドルを作成する
extern	int			LoadDivGraphFToResource2(			const TCHAR *ResourceName,                            const TCHAR *ResourceType,                            int AllNum, int XNum, int YNum, float XSize, float YSize, int *HandleArray ) ;	// 画像リソースを分割してグラフィックハンドルを作成する( float型 )
extern	int			LoadDivGraphFToResourceWithStrLen2(	const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength, int AllNum, int XNum, int YNum, float XSize, float YSize, int *HandleArray ) ;	// 画像リソースを分割してグラフィックハンドルを作成する( float型 )
...
extern	int			CreateFontDataFile(				const TCHAR *SaveFilePath,                            const TCHAR *FontName,                        int Size, int BitDepth /* DX_FONTIMAGE_BIT_1等 */ , int Thick, int Italic DEFAULTPARAM( = FALSE ) , int CharSet DEFAULTPARAM( = -1 ) , const TCHAR *SaveCharaList DEFAULTPARAM( = NULL )                                  ) ;				// フォントデータファイルを作成する
extern	int			CreateFontDataFileWithStrLen(	const TCHAR *SaveFilePath, size_t SaveFilePathLength, const TCHAR *FontName, size_t FontNameLength, int Size, int BitDepth /* DX_FONTIMAGE_BIT_1等 */ , int Thick, int Italic DEFAULTPARAM( = FALSE ) , int CharSet DEFAULTPARAM( = -1 ) , const TCHAR *SaveCharaList DEFAULTPARAM( = NULL ) , size_t SaveCharaListLength DEFAULTPARAM( = 0 ) ) ;				// フォントデータファイルを作成する
...
extern	int			CreateDIBGraph_plus_Alpha(					const TCHAR *FileName,                        HBITMAP *RGBBmp, HBITMAP *AlphaBmp,                                                                                                    int ReverseFlag DEFAULTPARAM( = FALSE ) , COLORDATA *SrcColor DEFAULTPARAM( = NULL ) ) ;	// 画像ファイルからＤＩＢデータとマスク用ＤＩＢデータを作成する
extern	int			CreateDIBGraph_plus_AlphaWithStrLen(		const TCHAR *FileName, size_t FileNameLength, HBITMAP *RGBBmp, HBITMAP *AlphaBmp,                                                                                                    int ReverseFlag DEFAULTPARAM( = FALSE ) , COLORDATA *SrcColor DEFAULTPARAM( = NULL ) ) ;	// 画像ファイルからＤＩＢデータとマスク用ＤＩＢデータを作成する
...
extern	int			ConvGraphImageToBitmap(						const BASEIMAGE *GraphImage, BITMAPINFO *BmpInfo, void **GraphData, int CopyFlag, int FullColorConv DEFAULTPARAM( = TRUE ) ) ;																								// 基本イメージデータをＢＭＰデータに変換するGraphImage を ＢＭＰ に変換する(アルファデータはあっても無視される)( 戻り値　0:正常終了  1:コピーを行った  -1:エラー )
...
// サウンドデータ管理系関数
extern	int			LoadSoundMemByResource(              const TCHAR *ResourceName,                            const TCHAR *ResourceType,                            int BufferNum DEFAULTPARAM( = 1 ) ) ;		// サウンドリソースからサウンドハンドルを作成する
extern	int			LoadSoundMemByResourceWithStrLen(    const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength, int BufferNum DEFAULTPARAM( = 1 ) ) ;		// サウンドリソースからサウンドハンドルを作成する
